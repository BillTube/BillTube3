...
BTFW.define("feature:styleCore", [], async () => {
  function ensureSlate(){
    const has = Array.from(document.querySelectorAll('link[rel="stylesheet"]')).some(l => /(bootstrap.*\.css|bootswatch.*slate)/i.test(l.href||""));
    if (has) return;
    if (!document.querySelector('link[data-btfw-slate]')) {
