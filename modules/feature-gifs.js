BTFW.define("feature:gifs", ["feature:chat"], async ({}) => {
  ...
  function switchTab(tab){
    document.querySelectorAll("#btfw-gif-modal .tabs li").forEach(li => li.classList.toggle("is-active", li.dataset.tab===tab));
    document.querySelectorAll("#btfw-gif-panes .pane").forEach(p => p.classList.toggle("is-hidden", p.dataset.pane!==tab));
  }
  ...
  function insertText(text){
    ...
    const s=line.selectionStart||line.value.length; const e=line.selectionEnd||line.value.length;
    line.value=line.value.slice(0,s)+text+line.value.slice(e);
    line.focus(); line.setSelectionRange(s+text.length,s+text.length);
  }
  ...
  async function perform(trending){
    ...
    }catch(e){ pane.innerHTML="<p>Failed to load GIFs.</p>"; }
  }

  function render(urls, pane){
    const g=document.createElement("div"); g.className="columns is-multiline is-mobile";
  ...
  // Wire events + export a public opener
  document.addEventListener("btfw:openGifs", show);
  window.BTFW_openGifs = show;
