  ...
  async function perform(trending){
    ...
    try{
      if(active==="giphy"){
        ...
        const ep = trending? "trending?limit=50" : ("search?q="+encodeURIComponent(q)+"&limit=50");
        const res=await fetch(base+ep+"&api_key="+GIPHY_KEY); const data=await res.json();
        const list=(data.data||[]).map(d=>trimGif(d.images?.downsized?.url||d.images?.original?.url));
        render(list, pane);
      }else{
        const base="https://api.tenor.com/v1/";
        const url = trending? (base+"trending?key="+TENOR_KEY+"&limit=50") : (base+"search?key="+TENOR_KEY+"&q="+encodeURIComponent(q)+"&limit=50");
        const res=await fetch(url); const data=await res.json();
        const list=(data.results||[]).map(r=>trimGif(r.media?.[0]?.gif?.url));
        render(list, pane);
      }
    }catch(e){ pane.innerHTML="<div class='btfw-grid-9'>Failed to load GIFs.</div>"; console.error(e); }
  }
