...
BTFW.define("feature:playlistCatalog", [], async () => {
  ...
  function injectStyles(){
    ...
    style.textContent = `
      ...
      #btfw-playlist-catalogue.is-vhs .btfw-catalogue__vhs { position:relative; display:block; width:100%; aspect-ratio:2/3; color:inherit; text-decoration:none; perspective:1000px; outline:none; filter:drop-shadow(9px 15px 10px rgba(0,0,0,.42)); transition:filter 250ms ease,transform 250ms cubic-bezier(.22,1,.36,1); }
      #btfw-playlist-catalogue .btfw-catalogue__vhs-rotor { position:absolute; inset:0; display:block; transform:rotateY(5deg) rotateX(1deg); transform-style:preserve-3d; transform-origin:center; transition:transform 400ms cubic-bezier(.22,1,.36,1); }
      #btfw-playlist-catalogue .btfw-catalogue__vhs-face { position:absolute; inset:0; display:block; overflow:hidden; border:1px solid rgba(255,255,255,.2); border-radius:4px 8px 8px 4px; backface-visibility:hidden; -webkit-backface-visibility:hidden; box-shadow:inset 1px 0 0 rgba(255,255,255,.13),inset -7px 0 9px rgba(0,0,0,.36),0 10px 18px rgba(0,0,0,.45); }
      #btfw-playlist-catalogue .btfw-catalogue__vhs-front { background:#101216; }
      #btfw-playlist-catalogue .btfw-catalogue__vhs-front::before { content:""; position:absolute; z-index:3; inset:0 auto 0 0; width:8%; border-right:1px solid rgba(255,255,255,.13); background:linear-gradient(90deg,rgba(0,0,0,.28),rgba(255,255,255,.12) 52%,rgba(0,0,0,.36)); }
      #btfw-playlist-catalogue .btfw-catalogue__vhs-front::after { content:""; position:absolute; z-index:4; inset:0; border-radius:inherit; background:linear-gradient(112deg,rgba(255,255,255,.16),transparent 18%,transparent 70%,rgba(0,0,0,.2)); box-shadow:inset 0 0 0 3px rgba(255,255,255,.045); pointer-events:none; }
      #btfw-playlist-catalogue .btfw-catalogue__vhs-label { position:absolute; z-index:1; inset:0 0 0 8%; overflow:hidden; display:block; background:linear-gradient(135deg,#343944,#14171d); }
      ...
      #btfw-playlist-catalogue .btfw-catalogue__vhs-back { padding:7%; transform:rotateY(180deg); color:#25221d; background:linear-gradient(145deg,#2b2722,#13110f); }
      ...
      #btfw-playlist-catalogue .btfw-catalogue__vhs-title { display:block; overflow:hidden; color:#1d1a16; font-size:clamp(.76rem,1vw,.9rem); font-weight:800; line-height:1.15; white-space:nowrap; text-overflow:ellipsis; }
