...
BTFW.define("feature:playlistCatalog", [], async () => {
  ...
  function injectStyles(){
    ...
    style.textContent = `
      ...
      #btfw-playlist-catalogue .btfw-catalogue__filters { display:grid; grid-template-columns:minmax(210px,1fr) minmax(130px,.46fr) minmax(150px,.56fr); gap:10px; padding:14px 20px 16px; border-bottom:1px solid color-mix(in srgb,var(--btfw-border,#2a2f3a) 72%,transparent); background-color:color-mix(in srgb,var(--btfw-color-panel,#171d2b) 82%,transparent); background-image:var(--btfw-panel-gradient-soft); background-size:var(--btfw-panel-background-size); background-position:var(--btfw-panel-background-position); }
