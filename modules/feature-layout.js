BTFW.define("feature:layout", ["feature:styleCore","feature:bulma"], async ({ require }) => {
  ...
  function stripClassesOn(el){
    ...
    const rm = [];
    el.classList.forEach(c => { if (BOOT.test(c)) rm.push(c); });
...
function killVideoHeader(){
  ...
  /** Build the 3-col grid: left | splitter | right */
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", function(){ ensureShell(); wireTopOffset(); ready(); });
  } else {
    ensureShell(); wireTopOffset(); ready();
  }
