...
BTFW.define("feature:layout", ["feature:styleCore","feature:bulma"], async ({}) => {
  function varTop(){ const h=(document.querySelector(".navbar,#nav-collapsible,#navbar,.navbar-fixed-top")||{}).offsetHeight||48; document.documentElement.style.setProperty("--btfw-top", h+"px"); }
  ...
  function ensureShell(){
    ...
    if(!document.getElementById("btfw-grid")){
      ...
      const left=document.createElement("div"); left.id="btfw-leftpad"; const right=document.createElement("aside"); right.id="btfw-chatcol";
      if(v) left.appendChild(v); if(q) left.appendChild(q); if(c) right.appendChild(c);
      grid.appendChild(left); const split=document.createElement("div"); split.id="btfw-vsplit"; grid.appendChild(split); grid.appendChild(right); wrap.prepend(grid);
    } else {
  ...
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", function(){ ensureShell(); varTop(); ready(); });
  else { ensureShell(); varTop(); ready(); }
  setTimeout(varTop, 500);
