BTFW.define("feature:channelThemeAdmin", [], async () => {
  ...
  function renderGradientLayer(theme, strengthScale = 1){
    ...
    if (gradient.type === "flow") {
      ...
      const fields = stops.map((stop, index) => `<radialGradient id="flow${index}"><stop offset="0" stop-color="${stop.color}" stop-opacity="1"/><stop offset=".58" stop-color="${stop.color}" stop-opacity=".94"/><stop offset="1" stop-color="${stop.color}" stop-opacity="0"/></radialGradient>`).join("");
      const shapes = `<ellipse class="flow-field flow-3" cx="900" cy="390" rx="760" ry="535" fill="url(#flow3)"/><ellipse class="flow-field flow-1" cx="235" cy="475" rx="690" ry="455" fill="url(#flow1)"/><ellipse class="flow-field flow-2" cx="1060" cy="500" rx="650" ry="430" fill="url(#flow2)"/><ellipse class="flow-field flow-0" cx="450" cy="-70" rx="820" ry="390" fill="url(#flow0)"/>`;
      return gradientSvgLayer(`${motionStyle}<defs>${fields}<filter id="f" filterUnits="userSpaceOnUse" x="-280" y="-240" width="1760" height="1200"><feTurbulence type="fractalNoise" baseFrequency=".004 .008" numOctaves="3" seed="17" result="n"/><feDisplacementMap in="SourceGraphic" in2="n" scale="${displacement}" xChannelSelector="R" yChannelSelector="B"/><feGaussianBlur stdDeviation="${blur}"/></filter></defs><g opacity="${opacity}"><rect width="1200" height="720" fill="${stops[0].color}"/><g filter="url(#f)">${shapes}</g></g>`);
    }
