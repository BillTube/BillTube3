/* BTFW — feature:js-editor
   Turns the Channel JS editor (#cs-jstext, a plain <textarea>) into a
   syntax-highlighted code editor (CodeMirror 5, dark theme matched to the
   BillTube UI) with a live JavaScript checker (JSHint).
   ...
   • Save gate: when you hit "Save JS" with real errors, we intercept and show a
     themed confirm listing each error + line ("Save anyway" / "Go back & fix").
     Warnings are shown but never block the save.

   CodeMirror + JSHint are lazy-loaded from jsDelivr the first time the JS editor
   becomes visible, so normal page loads are unaffected. The underlying <textarea>
   stays in sync (cm.save()), so CyTube's own save path is unchanged.

   Strictness = "errors + key warnings": undefined vars, '=' vs '==', unreachable
   code; missing semicolons are NOT nagged (asi:true) to keep the noise down.
*/
BTFW.define("feature:js-editor", [], async () => {
  ...
  let cm = null, libsPromise = null, allowSaveOnce = false, statusEl = null, statusTimer = null;
  ...
  // Common CyTube + BillTube globals so legit channel JS doesn't get false "not defined".
  ...
  function ensureLibs(){
    if (window.CodeMirror && window.CodeMirror.modes && window.CodeMirror.modes.javascript && window.JSHINT) {
      injectSkin(); registerLint(); return Promise.resolve();
    }
    ...
    libsPromise = (async () => {
      ...
      await injectJs(`${CM}/mode/javascript/javascript.min.js`);
      await Promise.all([
  ...
  function registerLint(){
    if (!window.CodeMirror || window.CodeMirror._btfwLint) return;
    window.CodeMirror.registerHelper("lint", "javascript", function(text){
      const out = [];
      const { errors, warnings } = runJSHint(text);
      const push = (it, sev) => out.push({
      ...
      errors.forEach(e => push(e, "error"));
      warnings.forEach(w => push(w, "warning"));
      return out;
    });
    window.CodeMirror._btfwLint = true;
  }
  ...
  /* ---------------- editor ---------------- */
  function jsPaneVisible(){
    const ta = document.getElementById("cs-jstext");
  ...
  function initEditor(){
    ...
    if (!ta) return;
    if (cm) {
      if (cm.getTextArea && document.body.contains(cm.getTextArea())) { cm.refresh(); return; }
      cm = null; statusEl = null; // stale (modal recreated the textarea)
    }
    registerLint();
    cm = window.CodeMirror.fromTextArea(ta, {
      mode: "javascript", theme: "btfw-ide",
    ...
    statusEl.className = "btfw-ide__status";
    statusEl.innerHTML = '<span class="btfw-ide__pos">Ln 1, Col 1</span><span class="btfw-ide__lint ok">✓ No problems</span>';
    ide.appendChild(statusEl);

    cm.on("change", () => { cm.save(); scheduleStatus(); });
    cm.on("cursorActivity", updatePos);
    installSaveHook();
    setTimeout(() => { cm.refresh(); updateStatus(); updatePos(); }, 40);
  }
  function updatePos(){
    if (!cm || !statusEl) return;
    const c = cm.getCursor();
    const p = statusEl.querySelector(".btfw-ide__pos");
    if (p) p.textContent = `Ln ${c.line+1}, Col ${c.ch+1}`;
  }
  function updateStatus(){
    if (!cm || !statusEl) return;
    const { errors, warnings } = runJSHint(cm.getValue());
    const el = statusEl.querySelector(".btfw-ide__lint");
    if (!el) return;
  ...
  function scheduleStatus(){ clearTimeout(statusTimer); statusTimer = setTimeout(updateStatus, 350); }

  /* ---------------- save confirm dialog ---------------- */
  function esc(s){ return String(s).replace(/[&<>"]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c])); }
  function closeConfirm(){ const e = document.getElementById("btfw-ide-confirm"); if (e) e.remove(); }
  function showSaveConfirm(errors, warnings, onConfirm){
    closeConfirm();
    ...
    ov.innerHTML = `<div class="btfw-ide-confirm__card">
      <div class="btfw-ide-confirm__head">⚠ ${errors.length} JavaScript error${errors.length>1?"s":""} found</div>
    ...
    ov.querySelectorAll(".btfw-ide-confirm__list li[data-ln]").forEach(li => li.onclick = () => {
      closeConfirm();
      if (cm) { cm.focus(); cm.setCursor(+li.dataset.ln - 1, (+li.dataset.ch || 1) - 1); }
    });
    ov.querySelector(".btfw-ide-confirm__cancel").onclick = () => {
      closeConfirm();
      if (cm && errors[0]) { cm.focus(); cm.setCursor(errors[0].line - 1, (errors[0].ch || 1) - 1); }
    };
  ...
  /* ---------------- save gate (capture phase, runs before CyTube's handler) ----------------
     Installed lazily the first time the editor mounts, so viewers who never open
     the JS editor never carry a global click listener. */
  let saveHookInstalled = false;
  function installSaveHook(){
    if (saveHookInstalled) return;
    saveHookInstalled = true;
    document.addEventListener("click", function(e){
      const btn = e.target && e.target.closest && e.target.closest("#cs-jssubmit");
      if (!btn || !cm) return;
      cm.save(); // make sure the textarea has the latest content for CyTube
      if (allowSaveOnce) { allowSaveOnce = false; return; } // user already confirmed → let it through
      const { errors, warnings } = runJSHint(cm.getValue());
      if (!errors.length) return; // no real errors → let CyTube save (warnings don't block)
      e.preventDefault();
      e.stopImmediatePropagation();
      showSaveConfirm(errors, warnings, () => { allowSaveOnce = true; btn.click(); });
    }, true);
  }

  /* ---------------- lazy init wiring ---------------- */
  function maybeInit(){
    if (!jsPaneVisible()) return;
    ensureLibs().then(initEditor).catch(err => console.warn("[js-editor] lib load failed", err));
  }
  function attach(){
    ...
    if (modal && !modal._btfwJsObs) {
      ...
      const mo = new MutationObserver(() => {
        if (deb) return;
        deb = setTimeout(() => { deb = null; if (jsPaneVisible()) maybeInit(); }, 120);
      });
